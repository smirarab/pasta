...
Version 1.6.2:
    ...
    [Buf fix]   -gtr was hard-coded for FastTree when model was empty; now -gtr is NOT automatically added when a configuration file is used and this enables using JK model.  (has backward compatibility issues when a configuration file is used and model is not set in the configuration file)
...
Version 1.6 (has important backward compatibility implications):
